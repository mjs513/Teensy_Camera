...
//    g4=0xff, uint8_t g5=0xff,uint8_t g6=0xff,uint8_t g7=0xff);
  ...
  //
  // HM0360: 8bit mode
  //         (FLEXIO 15fps, QVGA): f, F, m, V works. NOTE: M does not(no hangs
  //         though) (GPIO   15, QVGA   ): f, F, m, V works. NOTE: M does not(no
  //         hangs though) (FLEXIO) QVGA works, QQVGA, works but M hangs it?
...
// int camera_width = Camera.width();
    ...
    case 'V': {
      if (!g_continuous_flex_mode) {
        if (camera.readContinuous(&camera_flexio_callback_video, frameBuffer,
                                  sizeof(frameBuffer), frameBuffer2,
                                  sizeof(frameBuffer2))) {
...
#ifdef CAMERA_USES_MONO_PALETTE
    Serial.printf("NP:%d W:%u H:%u %p\n", numPixels, FRAME_WIDTH, FRAME_HEIGHT,
                  frameBuffer);
    if (use_frame_buffer)
