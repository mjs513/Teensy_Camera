...
void dump_partial_FB() {
  ...
  for (volatile uint16_t *pfb =
    ...
    for (uint16_t i = 0; i < 8; i++)
      Serial.printf("%02x ", pfb[i]);
    Serial.print("..");
    for (uint16_t i = (camera.width() / 2) - 4; i < (camera.width() / 2) + 4;
         i++)
      Serial.printf("%02x ", pfb[i]);
    Serial.print("..");
    for (uint16_t i = camera.width() - 8; i < camera.width(); i++)
...
bool readJPG(uint8_t &eoi_jpg, uint32_t &eop_jpg, bool debug_on) {
  ...
  if (camera.usingGPIO()) {
    ...
    bytes_read = omni.readFrame(frameBuffer, sizeof_framebuffer, frameBuffer2,
                                sizeof_framebuffer2);
    // delay(1000);
    // camera.readFrame(frameBuffer, sizeof_framebuffer, frameBuffer2,
    // sizeof_framebuffer2);
  ...
  if (bytes_read < sizeof_framebuffer) {
    ...
    if (debug_on)
      MemoryHexDump(Serial, pfb2 + bytes_read_in_2 - bytes_dump + 1, bytes_dump,
                    true);
