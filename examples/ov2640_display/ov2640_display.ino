...
// This is mandatory to begin the d session.
    ...
    //    hsync_pin, en_pin, uint8_t g0, uint8_t g1,uint8_t g2, uint8_t g3,
    ...
    uint8_t status = 0;
    status =
        camera.begin(camera_framesize, camera_format, 15, CameraID, useGPIO);
    ...
    if (!status) {
        ...
        delay(500);
        status = camera.begin(camera_framesize, camera_format, 15, CameraID,
                              useGPIO);
        if (!status) {
    ...
    // GREEISH, BLUEISH, RETRO omni.setWBmode(0);                  // AWB ON, 1
    // - Sunny, 2
    ...
    if (Serial.available()) {
...
#if defined(USE_SDCARD)
        if ('2' == command)
            storage_index = CommandLineReadNextNumber(ch, 0);
#endif
        switch (command) {
        case 'p': {
#if defined(USB_DUAL_SERIAL) || defined(USB_TRIPLE_SERIAL)
        ...
        case 'V': {
            if (!g_continuous_flex_mode) {
                if (!tft.useFrameBuffer(true)) {
                ...
                } else if (camera.readContinuous(
                               &camera_flexio_callback_video, frameBuffer,
                               sizeof_framebuffer, frameBuffer2,
                               sizeof_framebuffer2)) {
                    ...
                    if (!tft.updateScreenAsync(true)) {
                        Serial.println(
                            "* error, could not start Update Screen Async\n");
                    }
        ...
        case '1':
            ...
            for (uint32_t ii = 0; ii < fsCount; ii++) {
                Serial.printf("store:%u storage:%x name:%s fs:%x pn:\n", ii,
                              MTP.Store2Storage(ii),
                              MTP.getFilesystemNameByIndex(ii),
        ...
        if (g_new_flexio_data && (!g_flex_dual_buffer_per_frame ||
            ...
            if (g_flex_dual_buffer_per_frame) {
                ...
                if (g_new_flexio_data == frameBuffer) {
                    ...
                    int numPixels2 = min((int)(sizeof_framebuffer2 / 2),
                                         numPixels - numPixels1);
                    for (int i = 0; i < numPixels2; i++)
                        frameBuffer2[i] = HTONS(frameBuffer2[i]);
                    int start_x = (tft.width() - camera.width()) / 2;
                    int start_y = ((tft.height() - camera.height()) / 2) +
...
bool send_jpeg() {
    ...
    // byte swap
    // for (int i = 0; i < numPixels; i++) frameBuffer[i] = (frameBuffer[i] >>
    // 8) | (((frameBuffer[i] & 0xff) << 8)); now see if all fit into one buffer
    // or part in second...
    ...
    if (0)
        Serial.printf("\tBuffers:%p(%u) %p(%u)\n", frameBuffer,
                      sizeof_framebuffer, frameBuffer2, sizeof_framebuffer2);
    if (0)
        Serial.printf("\tnumpixels %u %u %u\n", numPixels, numPixels1,
                      numPixels2);
    // for (int i = 0; i < numPixels1; i++) frameBuffer[i] =
    // HTONS(frameBuffer[i]); for (int i = 0; i < numPixels2; i++)
    // frameBuffer2[i] = HTONS(frameBuffer2[i]);
...
void showCommandList() {
    ...
    Serial.println(
        "Send the 'c' character to debug clock - print vsync timing");
    ...
    Serial.println(
        "Send the 'Q' character to print current JPEG Quality value");
    Serial.println(
        "Send the 'Q<value>' to change the quality 10-63 lower is higher");
...
void read_display_one_frame(bool use_dma, bool show_debug_info) {
    ...
    uint32_t count_pixels_in_buffer =
        sizeof_framebuffer / sizeof(frameBuffer[0]);
    if (show_debug_info) {
        Serial.println("Reading frame");
        Serial.printf("Buffer1: %p(%u) halfway: %p end:%p\n", frameBuffer,
                      sizeof_framebuffer,
                      &frameBuffer[count_pixels_in_buffer / 2],
...
// int camera_width = Camera.width();
  ...
  return;
    ...
    if (numPixels2 == 0) {
        ...
        // We used both buffers.  Assume for now each buffer is in multples of
        // Width
        //    tft.writeRect(CENTER, CENTER, camera.width(), camera.height(),
        //    frameBuffer);
        int start_x = (tft.width() - camera.width()) / 2;
...
void dump_partial_FB() {
    ...
    for (volatile uint16_t *pfb =
         ...
         pfb < (frameBuffer + camera.width() * (camera.height() / 2 + 8));
         pfb += camera.width()) {
        Serial.printf("\n%08x: ", (uint32_t)pfb);
        for (uint16_t i = 0; i < 8; i++)
            Serial.printf("%02x ", pfb[i]);
...
void panCameraWindow() {
    ...
    for (;;) {
        for (uint16_t start_y = 0;
             start_y < (camera.frameHeight() - camera.height());
             start_y += 100) {
            for (uint16_t start_x = 0;
                 start_x < (camera.frameWidth() - camera.width());
                 start_x += 100) {
                camera.setZoomWindow(start_x, start_y);
                read_display_one_frame(true, false);
                delay(250);
...
bool readJPG(uint8_t &eoi_jpg, uint32_t &eop_jpg, bool debug_on) {
    ...
    uint32_t numPixels2 =
        min((sizeof_framebuffer2 / 2), numPixels - numPixels1);
    if (debug_on)
        Serial.printf("\tBuffers:%p(%u) %p(%u)\n", frameBuffer,
                      sizeof_framebuffer, frameBuffer2, sizeof_framebuffer2);
    if (debug_on)
    ...
    if (jpegSize > (sizeof_framebuffer + sizeof_framebuffer2))
        return false;
    ...
    if (camera.usingGPIO()) {
        ...
        bytes_read = omni.readFrame(frameBuffer, sizeof_framebuffer,
                                    frameBuffer2, sizeof_framebuffer2);
        // delay(1000);
        // camera.readFrame(frameBuffer, sizeof_framebuffer, frameBuffer2,
        // sizeof_framebuffer2);
    ...
    if (debug_on)
        Serial.printf("Bytes returned from camera: %u\n", bytes_read);
    ...
    // verify that the start of data returned has valid marker... assumes always
    // in first buffer
    uint8_t *pfb = (uint8_t *)frameBuffer;
    if (debug_on)
        MemoryHexDump(Serial, frameBuffer, 128, true, "SOF:\n");
    if ((pfb[0] != 0xff) || (pfb[1] != 0xd8) || (pfb[2] != 0xff)) {
        if (debug_on)
            Serial.printf("begining of frame not found at position 0\n");
        return false;
    }
    ...
    if (bytes_read < sizeof_framebuffer) {
        if (debug_on)
            MemoryHexDump(Serial, pfb + bytes_read - 63, 64, true, "\nEOF:\n");
        if ((pfb[bytes_read - 2] != 0xFF) || (pfb[bytes_read - 1] != 0xd9)) {
            if (debug_on)
                Serial.printf("Invalid frame ending: %02x %02x\n",
        ...
        if (debug_on)
            MemoryHexDump(Serial, pfb2 + bytes_read_in_2 - bytes_dump + 1,
                          bytes_dump, true);
        if ((pfb2[bytes_read_in_2 - 2] != 0xFF) ||
            (pfb2[bytes_read_in_2 - 1] != 0xd9)) {
            if (debug_on)
                Serial.printf("Invalid frame ending(2): %02x %02x\n",
                              pfb[bytes_read_in_2 - 2],
                              pfb[bytes_read_in_2 - 1]);
            return false;
        }
    }
