...
void ov7670_configure(void *, int devtype, int format, int wsize,
                      int clock_speed, int pll_bypass, int pclk_hb_disable);
...
*/
// int OV767X::begin(int resolution, int format, int fps,  int camera_name, bool
// use_gpio)
bool OV767X::begin_omnivision(framesize_t resolution, pixformat_t format,
...
#endif
  ov7670_configure(_ov7670, camera_name /*OV7670 = 0, OV7675 = 1*/, _format,
                   _framesize, _xclk_freq /* MHz */, 0 /*pll bypass*/,
                   1 /* pclk_hb_disable */);
...
void OV767X::noTestPattern() { ov7670_s_test_pattern(_ov7670, 0); }
...
void OV767X::setContrast(int contrast) { ov7670_s_contrast(_ov7670, contrast); }
...
void OV767X::setGain(int gain) { ov7670_s_gain(_ov7670, gain); }

void OV767X::autoGain(int enable, float gain_db, float gain_db_ceiling) {
  ov7670_s_autogain(_ov7670, enable);
}

void OV767X::setExposure(int exposure) { ov7670_s_exp(_ov7670, exposure); }
...
#else
bool OV767X::startReadFrameDMA(bool (*callback)(void *frame_buffer),
...
void OV767X::captureFrameStatistics() {
  ...
  while ((*_vsyncPort & _vsyncMask) == 0)
    fstatOmni.vsyncStartCycleCount++; // wait for LOW
  ...
  while ((*_vsyncPort & _vsyncMask) != 0) {
    ...
    if (regs != regs_prev) {
      ...
      if ((regs & _pclkMask) && ((regs_prev & _pclkMask) == 0))
        fstatOmni.pclkCounts[fstatOmni.hrefCount]++;
      if ((regs & _pclkMask) && ((regs_prev & _hrefMask) == 0))
        fstatOmni.pclkNoHrefCount++;
      regs_prev = regs;
    }
  }
  while ((*_vsyncPort & _vsyncMask) == 0)
    fstatOmni.vsyncEndCycleCount++; // wait for LOW
  // interrupts();
  ...
  for (uint16_t ii = 0; ii < fstatOmni.hrefCount + 1; ii++) {
    debug.printf("%3u(%u) ", fstatOmni.pclkCounts[ii],
                 (ii == 0) ? 0
                           : fstatOmni.hrefStartTime[ii] -
                                 fstatOmni.hrefStartTime[ii - 1]);
    if (!(ii & 0x0f))
      debug.print("\n    ");
  }
  debug.println();
...
uint8_t OV767X::cameraReadRegister(uint8_t reg) {
  ...
  if (Wire.endTransmission(false) != 0) {
    if (_debug)
      debug.println("error reading OV767X, address");
    return 0;
  }
  if (Wire.requestFrom(0x42 >> 1, 1) < 1) {
    if (_debug)
      debug.println("error reading OV767X, data");
    return 0;
  }
  return Wire.read();
}

void OV767X::showRegisters() { ov7670_printRegs(); }
