...
void ImageSensor::setPins(uint8_t mclk_pin, uint8_t pclk_pin, uint8_t vsync_pin,
  ...
  debug.printf("void ImageSensor::setPins(MC:%u PC:%u vs:%u hr:%u en:%u, DP: "
               "%u %u %u %u %u %u %u %u W:%p)\n",
               mclk_pin, pclk_pin, vsync_pin, hsync_pin, en_pin, g0, g1, g2, g3,
               g4, g5, g6, g7, &wire);
#endif
  _vsyncPin = vsync_pin;
  _hrefPin = hsync_pin;
...
size_t ImageSensor::readFrameFlexIO(void *buffer, size_t cb1, void *buffer2,
  ...
  for (;;) {
    ...
    while ((*_vsyncPort & _vsyncMask) == 0) {
      if (emWaitSOF > _timeout) {
        if (_debug)
          debug.println("Timeout waiting for rising edge Start of Frame");
        return 0;
      }
    }
    emGlitch = 0;
    while ((*_vsyncPort & _vsyncMask) != 0) {
      if (emWaitSOF > _timeout) {
        if (_debug)
          debug.println("Timeout waiting for falling edge Start of Frame");
        return 0;
      }
    }
    if (emGlitch > 5)
    ...
    while (frame_bytes_received < frame_size_bytes) {
      while ((_pflexio->SHIFTSTAT & _fshifter_mask) == 0) {
        if (timeout > max_time_to_wait) {
          ...
          if (_debug)
            debug.printf("Timeout between characters: received: %u bytes\n",
      ...
      if (p >= p_end) {
        ...
        if (buffer2) {
          ...
          if (_debug && (frame_bytes_received < frame_size_bytes))
            Serial.printf("Error: Image size exceeded buffer space\n");
          break;
        }
      }
...
void ImageSensor::dumpDMA_TCD(DMABaseClass *dmabc, const char *psz_title) {
  ...
  debug.printf("SA:%x SO:%d AT:%x (SM:%x SS:%x DM:%x DS:%x) NB:%x SL:%d DA:%x "
               ...
               (uint32_t)dmabc->TCD->DADDR, dmabc->TCD->DOFF, dmabc->TCD->CITER,
               dmabc->TCD->DLASTSGA, dmabc->TCD->CSR, dmabc->TCD->BITER);
}
...
bool ImageSensor::flexio_configure() {
  ...
  if ((thsync_pin == 0xff) || (tg0 == 0xff) || (tg1 == 0xff) || (tg2 == 0xff) ||
      (tg3 == 0xff)) {
    debug.printf("OV767X Some pins did not map to valid Flex IO pin\n");
    if (_debug)
      debug.printf("    HSYNC(%u %u) G0(%u %u) G1(%u %u) G2(%u %u) G3(%u %u)",
                   _hrefPin, thsync_pin, _dPins[0], tg0, _dPins[1], tg1,
                   _dPins[2], tg2, _dPins[3], tg3);
    return false;
  }
  ...
  if ((tg1 != (tg0 + 1)) || (tg2 != (tg0 + 2)) || (tg3 != (tg0 + 3))) {
    ...
    if (_debug)
      debug.printf("    G0(%u %u) G1(%u %u) G2(%u %u) G3(%u %u)", _dPins[0],
                   tg0, _dPins[1], tg1, _dPins[2], tg2, _dPins[3], tg3);
    return false;
  }
  if (_dPins[4] != 0xff) {
...
#define FLEXIO_TIMER_TRIGGER_SEL_PININPUT(x) ((uint32_t)(x) << 1U)
  if (_hw_config == TEENSY_MICROMOD_FLEXIO_8BIT) {
    ...
    //          0 = 1 bit, 1-3 = 4 bit, 4-7 = 8 bit, 8-15 = 16 bit, 16-31 = 32
    //          bit
  ...
  // TIMCFG, page 2935
  //  TIMOUT: Output
  //          0 = output is logic one when enabled and is not affected by timer
  //          reset 1 = output is logic zero when enabled and is not affected by
  //          timer reset 2 = output is logic one when enabled and on timer
  //          reset 3 = output is logic zero when enabled and on timer reset
  //  TIMDEC: Decrement
  ...
  //          3 = on Trigger input (both edges), Shift clock equals Trigger
  //          input
  //  TIMRST: Reset
  //          0 = never reset
  ...
  _pflexio->TIMCFG[_ftimer] = FLEXIO_TIMCFG_TIMOUT(1) |
                              FLEXIO_TIMCFG_TIMDEC(2) |
