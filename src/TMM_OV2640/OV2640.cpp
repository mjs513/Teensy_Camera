...
int OV2640::setBrightness(int level) {
  ...
  for (int i = 0; i < NUM_BRIGHTNESS_LEVELS; i++) {
    ret |=
        cameraWriteRegister(brightness_regs[0][i], brightness_regs[level][i]);
  }
...
void OV2640::setSaturation(int level) {
  ...
  for (int i = 0; i < NUM_SATURATION_LEVELS; i++) {
    ret |=
        cameraWriteRegister(saturation_regs[0][i], saturation_regs[level][i]);
  }
...
void OV2640::showRegisters(void) {
  ...
  for (uint16_t ii = 0;
       ii < (sizeof(OV2640_reg_name_table) / sizeof(OV2640_reg_name_table[0]));
       ii++) {
